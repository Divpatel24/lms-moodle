...
@app.route('/admin/ajax_course_student')
def admin_ajax_course_student():
    try:
        if admin_login_session() == "admin":
            course_vo = CourseVO()
            course_dao = CourseDAO()
            course_degree_id = request.args.get('studentDegreeId')
            course_vo.course_degree_id = course_degree_id
            course_vo_list = course_dao.view_ajax_course(
                course_vo)
            ajax_student_course = [i.as_dict() for i in course_vo_list]
            return jsonify(ajax_student_course)
        else:
            return admin_logout_session()
    except Exception as ex:
        print("admin_ajax_course_student route exception "
              "occured>>>>>>>>>>", ex)
...
def admin_ajax_semester_student():
    try:
        if admin_login_session() == "admin":
            ...
            semester_course_id = request.args.get('studentCourseId')
...
def admin_insert_student():
    try:
        if admin_login_session() == "admin":
            ...
            student_course_id = request.form.get('studentCourseId')
            ...
            student_vo.student_course_id = student_course_id
...
def faculty_view_student():
    try:
        if admin_login_session() == "faculty":
            ...
            student_dao = StudentDAO()
            student_vo_list = student_dao.view_studentdata(faculty_course_id)
