...
@app.route('/admin/ajax_course_faculty')
def admin_ajax_course_faculty():
    try:
        if admin_login_session() == "admin":
            course_vo = CourseVO()
            course_dao = CourseDAO()
            course_degree_id = request.args.get('facultyDegreeId')
            course_vo.course_degree_id = course_degree_id
            course_vo_list = course_dao.view_ajax_course(
                course_vo)
            ajax_faculty_course = [i.as_dict() for i in course_vo_list]
            return jsonify(ajax_faculty_course)
        else:
            return admin_logout_session()
    except Exception as ex:
        print("admin_ajax_course_Faculty route exception "
              "occured>>>>>>>>>>", ex)
...
def admin_ajax_subject_faculty():
    try:
        if admin_login_session() == "admin":
            ...
            subject_course_id = request.args.get('facultyCourseId')
...
def admin_insert_faculty():
    try:
        if admin_login_session() == "admin":
            ...
            faculty_course_id = request.form.get('facultyCourseId')
            ...
            faculty_vo.faculty_course_id = faculty_course_id
...
def faculty_edit_profile():
    try:
        if admin_login_session() == "faculty":
            ...
            # bringing subjects based on course id
            subject_vo = SubjectVO()
            subject_dao = SubjectDAO()
            subject_vo.subject_course_id = faculty_course_id
